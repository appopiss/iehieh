using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.UI;
using static UsefulMethod;
using System;

public class B_graduate : B_Upgrade
{
    private void Awake()
    {
        awakeText();
        thisId = UpgradeId.graduate;
        RequiredReputation = 1000;
        IncrementReputation = 10;
    }
    // Start is called before the first frame update
    void Start()
    {
        StartUpgrade();
        gameObject.GetComponent<Button>().onClick.AddListener(calculate);
    }

    //Cost: 100000*Math.Pow(3,SBank.MonsterCounterLevel)
    //ROUNDUP(150000*POWER(1.1,MAX(FLOOR([UpgradeLevel]-1,10)/10+1,1)),-4)
    public override double calculateCurrentCost(int level)
    {
        return 10000 * Math.Pow(2.5, level) * (1 - costFactor);
    }

    public override double calculateCurrentValue()
    {
        return Math.Pow(2, level);
    }
    public override double calculateCurrentValue(int level)
    {
        return Math.Pow(2, level);
    }

    // Update is called once per frame
    void Update()
    {
        UpdateUpgrade();
        checkButton();
        if (!window.activeSelf)
            return;
        BankLocal.B_graduate(ref Name, ref effectExplain, ref explain);

        currentValue = "Train I factor * " + tDigit(calculateCurrentValue());
        nextValue = "Train I factor * " + tDigit(calculateNextValue());
    }
}
