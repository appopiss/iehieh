...
public class B_autoStone : B_Upgrade
{
    private void Awake()
    {
        awakeText();
    ...
    void Update()
    {
        ...
        if (!window.activeSelf)
            return;
        Name = "Auto Stone Upgrades";
        effectExplain = "You can randomly automatically buy stone upgrades.";
        if(level==0)
            currentValue = "Locked";
        else
            currentValue = "Every " + tDigit(122 * Mathf.Pow(0.99f, level)) + "s";
        nextValue = "Every " + tDigit(122 * Mathf.Pow(0.99f, level+1)) + "s";
        explain = "An old man sits off to the side of the circular room. Approaching him, he quickly swaps his hat, which has an emblem for Stone on it. Before you can speak to question the legitimacy of his operation, he starts talking to you. \"Hey there sonny! I\'m head of the Stone labor union! I\'ve got an exciting time share.. wait, wrong spiel.. I\'ve got an exciting opportunity for you! Do you constantly run out of gold to hire more workers to harvest your Stone? Well let me tell you, I\'ve got loads of unemployed children, bears, and even golems just waiting to be hired for free. You, uhh, just have to pay the low, low, chance of a lifetime fee of 100,000 SC. Real slick deal, I\'m telling you. So are you interested?\" It's very clear he never saved a penny towards retirement.";
    }

    public M_UPGRADE chooseUpgrade()
    {
        List<M_UPGRADE> tempList = new List<M_UPGRADE>();
        foreach (Transform child in main.bankCtrl.StoneUpgradeCanvas)
        {
    ...
    public IEnumerator AutoBuyStone()
    {
        while (true)
        {
            ...
            yield return new WaitForSeconds(120f * Mathf.Pow(0.99f, level));
        }
    }
