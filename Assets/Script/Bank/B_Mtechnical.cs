...
public class B_Mtechnical : B_Upgrade
{
    private void Awake()
    {
        awakeText();
        thisId = UpgradeId.M_technical;
    ...
    void Update()
    {
        ...
        if (!window.activeSelf)
            return;
        BankLocal.B_mtech(ref Name, ref effectExplain, ref explain);

        currentValue = "Base MATK + " + tDigit(TotalEffect());
        nextValue = "Base MATK + " + tDigit(calculateNextValueTotal());
    }
}
