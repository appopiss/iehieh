...
public class C_leaf3 : M_UPGRADE
{
    public override int level { get => main.SR.LC_leaf3; set => main.SR.LC_leaf3 = value; }
    ...
    void Update()
    {
        ...
        if (!window.activeSelf)
            return;
        upgradeIcon.sprite = main.UpLeafSpriteAry[2];
        currentValue = tDigit(calculateCurrentValue(), 1) + "%";
        nextValue = tDigit(calculateNextValue(), 1) + "%";
        L_Upgrades.C_Leaf3(ref Name, ref effectExplain, ref explain);
    }
}
